...
This **Director** owns routing, platform strategy, compliance, and publishing checks, and orchestrates specialist sub-agents. It does **not** write the copy or render images itself — it delegates:

- **Copy production** (titles, body, carousel/opinion copy, tags, hooks) → `dependencies/xhs-copy/` (the copy sub-agent).
- **Cover / images** → `dependencies/xhs-image/` (edit a real photo, or generate from scratch).
- **Video** → `video-use`; **motion / HTML video** → `hyperframes` (external; see Dependency preflight).

Two mandatory quality gates:

1. **Copy gate**: every final title, cover text, caption, script, first comment, carousel text, and post copy passes the humanizer in `dependencies/xhs-copy/humanizer/SKILL.md` (applied by xhs-copy).
...
The platform/compliance rulebook below is the Director's strategy layer. Apply it after routing, and pass the relevant constraints down to the sub-agents.
...
All copy is produced by the **xhs-copy** sub-agent (`dependencies/xhs-copy/SKILL.md`), which runs its own humanizer as the final pass:

`dependencies/xhs-copy/humanizer/SKILL.md`

This gate applies to platform titles, cover text, captions, body copy, video scripts, voiceover, carousel page text, first comments, and interaction prompts. Do not deliver copy that has not passed it — even if the agent cannot load a separate skill, read the bundled file and apply the rules manually.
...
This Director is the **strategy + routing brain**. Copy and image production are delegated to sub-agents. Bundled in this repo (no install): `dependencies/xhs-copy/` (copy + humanizer) and `dependencies/xhs-image/` (cover). Heavy production skills (`video-use`, `hyperframes`) are external. So before delegating to any companion skill below, run the preflight protocol:
...
This Director owns **routing + strategy + compliance** (what angle, why, how to dodge limits). It does **not** own copy production or visual rendering. Delegate to the right sub-agent (after running the dependency preflight above):

| User intent | Skill to invoke | Why |
|---|---|---|
| Write / edit any post copy (titles, body, tags, hooks, captions) | `xhs-copy` | All text production lives in the copy sub-agent; the Director passes the brief + compliance constraints |
| Produce photo-carousel page copy | `xhs-copy` | Cover headline, slide titles, page body, caption, and tags are copy production |
| Cover from a real photo, or generate a cover from scratch | `xhs-image` | Edit mode preserves a real face; generate mode for no-face covers. Wording comes from `xhs-copy` |
...
| Big-title / motion cover when no photo | `xhs-image` (generate) or `hyperframes` + `animejs` / `gsap` | Visual rendering only; copy wording stays with `xhs-copy` |
...
5. **Cover + copy** → once the cut is approved, delegate copy to `xhs-copy` (title/body/tags) and cover candidates to `xhs-image`, then assemble `post.md`.
...
1. **Copy** → delegate to `xhs-copy` (Photo-carousel mode) to produce cover text, per-slide text, body, and tags
2. **Cover/carousel visuals** → delegate to `xhs-image` (edit real photos) or `hyperframes`
...
所有文案写作都在 **xhs-copy** 子 Agent：标题公式、正文结构模板、图文轮播文案、观点/叙事文、标签策略、互动钩、以及文案输出模板，全部在 `dependencies/xhs-copy/SKILL.md`。

Director 的职责是：路由 + 把 brief 和**本文「审核合规规则」的红线约束**传给 `xhs-copy`，并在交付前确认文案过了 humanizer 闸门。具体怎么写，读 `dependencies/xhs-copy/SKILL.md`。
...
## 互动钩（CES 优化）→ 交给 xhs-copy

评论钩（4×）/ 关注钩（8×）/ 完播钩 的模板都在 `dependencies/xhs-copy/SKILL.md`。Director 只需确认每条笔记"双钩齐发"，具体措辞由 xhs-copy 产出。
...
- [ ] 封面 3:4 或 1:1，主体清晰，标题大字（用 `xhs-image` 出封面，或 `hyperframes` 渲染单帧）
...
### When user wants copy (text only)

文案输出模板（copy-only / 图文轮播 / 观点叙事文 三种）都在 `dependencies/xhs-copy/SKILL.md` 的 "Copy output templates"。Director 把 brief + 合规约束传给 `xhs-copy`，由它按对应模板产出，再过 humanizer 闸门后交回。
...
6. **Generate cover candidates + `post.md`** after the cut is approved. Delegate copy to `xhs-copy` (title/body/tags) and cover to `xhs-image`.
...
- ❌ 在这个 skill 里手写 PIL / Canvas 画封面 —— 永远先 delegate 给 `xhs-image` / `hyperframes` 生态
- ❌ 在 Director 里直接写标题/正文/标签 —— 永远先 delegate 给 `xhs-copy`
